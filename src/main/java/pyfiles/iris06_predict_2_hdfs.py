...
def map_fun(args, ctx):
  ...
  if job_name == "ps":
    ...
    if args.mode == "train":
        print("not supported in predict mode !")
    else:
      ...
      while not sv.should_stop() and not tf_feed.should_stop() and step < args.steps:
        ...
        if len(batch_xs) > 0:
          if args.mode == "train":
            ...
            # results = ["Data: {0} , Label: {1}, Prediction: {2}".format(x, l, p)
            #            for x, l, p in zip(xs, labels, preds)] ## test not passed
            results = [" Label: {0}, Prediction: {1}".format(l, p)
...
if __name__ == '__main__':
    ...
    labelRDD = cluster.inference(input_data)
    print("labelRDD.count", labelRDD.count())
